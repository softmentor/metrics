...
public class InstrumentedJedisPoolTest {
  ...
  public void setup() {
    ...
    this.threadPool =
        new InstrumentedThreadPoolExecutor(poolSize, maxPoolSize, keepAlive, TimeUnit.SECONDS,
            new ArrayBlockingQueue<Runnable>(2), threadFactory, rejectionHandler, "test");
  ...
  public void testJedisPoolMetrics() throws Exception {
    ...
    gaugeActiveCount = (Gauge<Integer>) registry.getAllMetrics().get(this.threadPool.activeThreads);
    gaugeCorePoolSize = (Gauge<Integer>) registry.getAllMetrics().get(this.threadPool.corePoolSize);
    gaugeCurrentPoolSize =
        (Gauge<Integer>) registry.getAllMetrics().get(this.threadPool.currentPoolSize);
    gaugePercentIdle = (Gauge<Integer>) registry.getAllMetrics().get(this.threadPool.percentIdle);
    gaugePendingJobs = (Gauge<Integer>) registry.getAllMetrics().get(this.threadPool.pendingJobs);
    tasksExecTimer = (Timer) registry.getAllMetrics().get(this.threadPool.tasksExecuted);
    workerRunTimer = (Timer) registry.getAllMetrics().get(this.threadPool.workerTimeMetric);
    ...
    private void processCommand() {
      try {
        ...
        for (int i = 0; i <= maxRedisWrites; i++) {
          ...
          jedisProxy.zadd(testKey, System.currentTimeMillis(), testValue);
          // Thread.sleep(100);
        }
        ...
        System.out
            .println(String
                .format(
                    ...
                    this.executor.isTerminated()));
        assertThat(this.executor.getPoolSize(), Matchers.is(gaugeCurrentPoolSize.getValue()));
        assertThat(this.executor.getCorePoolSize(), Matchers.is(gaugeCorePoolSize.getValue()));
        assertThat(this.executor.getActiveCount(), Matchers.is(gaugeActiveCount.getValue()));
        System.out.println(String.format(
            "[sysout-monitor-metric-submittasktimer] Min: %s, Max: %s, Mean: %s, Count: %d",
            tasksExecTimer.getMin(), tasksExecTimer.getMax(), tasksExecTimer.getMean(),
            tasksExecTimer.getCount()));
        System.out.println(String.format(
            "[sysout-monitor-metric-workertimer] Min: %s, Max: %s, Mean: %s, Count: %d m1: %s",
            workerRunTimer.getMin(), workerRunTimer.getMax(), workerRunTimer.getMean(),
            workerRunTimer.getCount(), workerRunTimer.getMeanRate()));
